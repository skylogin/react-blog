...
export default React.createClass({
  ...
  createUser: function(e){
    ...
    Array.prototype.forEach.call(this.refs.form.querySelectorAll('input'), function(v){
      let fieldName = v.getAttribute('name'), errors;
      detail[fieldName] = v.value;

      errors = fieldName === 'username'?
        this.validateField(fieldName, update(this.constraints.username, {
          exclusive: {
            $set: this.state.users.map(function(v){
              return v.username;
            })
          }
        })):
  ...
  userImageUpload: function(e){
    ...
    render.onload = this.imageLoadedHandler;
      ...
      <form ref="form"
        ...
        <fieldset>
          ...
          <div className="profile-image-container">
            ...
            <BasicInput name="profileImage" type="file" ref="profileImage"
              onChange={this.userImageUpload} helptext={this.state.sizeExceeded? 'less than 1MB': ''}>
              <button onClick={this.chooseFile}>choose file</button>
            </BasicInput>
          </div>

          <BasicInput name="firstname" type="text" placeholder="first name" />
          <BasicInput name="lastname" type="text" placeholder="last name" />
